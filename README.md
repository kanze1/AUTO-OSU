...
当前应用版本为 **0.2.0**，节奏和坐标模型为 **v0**，已经能出让我自己愿意打完整首的图：节奏卡在鼓点上，跳和串都是从十几万张 ranked / approved / loved 谱面里学来的，
...
- **来源识别。** 当前输出带有 `autoosu ai-generated kanzei` 标签，软件尚无来源检查入口。标签可以修改，且当前规则模式也共用此标签，不能单凭它确认用了哪个模型；新版将区分生成引擎，并研究旧版 v0 结果的识别方法。
...
| P0 · 来源识别 | 新谱面生成记录、模型身份与内容指纹，软件中的来源检查入口；旧版 v0 检测可行性实验 | 支持 `.osu` / `.osz`；分别报告标签、记录匹配与检测实验的误报/召回 | 计划中 |
