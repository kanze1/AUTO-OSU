...
已发布应用为 **0.2.0**，当前源码开发版为 **0.3.0.dev0**，节奏和坐标模型仍为 **v0**。已经能出让我自己愿意打完整首的图：节奏卡在鼓点上，跳和串都是从十几万张 ranked / approved / loved 谱面里学来的，
...
| 星级条件 | 给模型的难度提示，留空按难度默认：Easy 2.0 / Normal 3.2 / Hard 4.5 / Insane 5.5。它不等于生成后的实际星级，实际星级测量与校准在开发排期中。 |
...
| `--check-source 路径` | 检查 `.osu` / `.osz` / 文件夹的来源声明与本机生成记录（源码开发版） |
...
新生成的谱包包含模型身份、参数与内容指纹，本机另存生成记录。检查结果区分「包含来源声明」「匹配本机记录」「无法判断」。标识可以被复制，没有匹配记录也不代表人工制作。统计检测尚未上线；[第一轮 v0 实验](docs/detection-v0-screen.md) 在少量编辑后失效，并把部分纯规则结果误归因给模型，因此没有接入软件。
...
- **来源识别。** 源码开发版已区分双模型、混合与纯规则输出，并提供声明和本机记录检查。旧版 0.2.0 的 `ai-generated` 标签也用于纯规则结果，不能据此确认用过模型；删标识旧图的统计检测仍未达到接入条件。
...
| P0 · 来源识别 | 新谱面生成记录、模型身份与内容指纹，软件中的来源检查入口；旧版 v0 检测可行性实验 | 支持 `.osu` / `.osz`；分别报告标签、记录匹配与检测实验的误报/召回 | 来源检查已实现在源码；统计实验未通过接入门槛 |
| P1 · 难度控制 | 实际星级测量、固定评估曲集、高难度密度与跨度控制 | 对比目标与实际星级，先验证 6–7★，再扩展 7–8★，同时检查局部难度峰值和手感 | 计划中 |
| P1 · 段落表现 | highlight 段落控制与自动定位 | 先验证手工区间，再做自动定位；密度、跨度、音效和 kiai 共同表现铺垫、爆发与回落 | 计划中 |
| P2 · 模型迭代 | 高星数据、细分节奏、滑条长度与段落条件、多红线 timing | 对照实验和固定曲集验证有提升，再更新发布模型 | 待前序实验结果 |

旧图统计检测只有通过独立的低误报评估后才会进入软件；未检出显示“无法判断”，不代表人工制作。
更细的修改范围、依赖和验收条件见 [任务安排与分析](docs/roadmap-2026-09-24.md)。
